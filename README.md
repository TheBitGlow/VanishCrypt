...
| 🗑️ **SSD 优化安全擦除** | 专为固态硬盘机制优化：重命名断开文件路径索引 → 随机密钥 AES 密文覆写原块 → 移除双临时文件，最大程度抵御磨损均衡层的数据残留。 |
| 📁 **批量与目录递归** | 支持同时选中多文件，或递归扫描深层文件夹，自动计算总体积与文件级 SHA-256 指纹。 |
| 🖱️ **拖放交互支持** | 直接从资源管理器拖放任意数量的文件或文件夹到窗口，即刻载入任务队列。 |
| 🔑 **强制密码合规检测** | 强制要求密码长度 ≥ 12 位，且必须同时包含大小写字母、数字及特殊字符；配备实时颜色反馈的动态密码强度指示器。 |
| 🛡️ **防呆双重确认** | 加密阶段要求二次确认密码；任何操作执行前弹出不可撤销风险告警，防止误操作。 |
...
- **擦除流程**：
  ...
  2. 生成全新的单次 256 位密钥与随机 Nonce，以 AES-GCM 密文流完整覆写该文件内容至 `tmp_enc`。
  3. 分别删除 `tmp_src` 与 `tmp_enc`。
- **物理局限说明**：
  > [!WARNING]
